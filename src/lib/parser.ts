...
export function getTopLevelReadableElementsOnPage() {
  ...
  return readAllElements(topLevelElements);
}

function readAllElements(elements: Element) {
  ...
  topLevelElements.forEach((element) => {
    if (!IGNORE_LIST.includes(element.tagName)) {
      if (
        ...
        const readableChild = readAllElements(element);
        for (const ele of readableChild) {
